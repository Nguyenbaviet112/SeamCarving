	...
	int * energy)
{
	...
	for (int filterR = 0; filterR < filterWidth; filterR++)
	{
		for (int filterC = 0; filterC < filterWidth; filterC++)
		{
			...
			int inPixelsR = r - temp + filterR;
			int inPixelsC = c - temp + filterC;
        	...
        	int times = 1)
{
	...
	for(int count = 0; count < times; count++)
	{
		...
		edgeDetectionKernel<<<gridSize, blockSize>>>(d_inGrayscale, width, height, d_filterX, d_filterY, filterWidth, d_energy);
		cudaDeviceSynchronize();
		CHECK(cudaGetLastError());
		...
		CHECK(cudaMemcpy(inPixels, d_outPixels, width*height*3*sizeof(uint8_t), cudaMemcpyDeviceToHost));

 
		
	}
	memcpy(outPixels, inPixels, width*height*3*sizeof(uint8_t));
	writePnm(outPixels, 3, width, height, "v1_device_out.pnm");
}
...
int main(int argc, char ** argv)
{
	...
	uint8_t * inPixels, * inPixels_device;
